...
function nwBlogs() {
	if (jQuery('#networkedblogs_nwidget_container').length > 0) {
		jQuery.getScript(
				"http://nwidget.networkedblogs.com/getnetworkwidget?bid=52780",
				function() {
		...
		jQuery
				.getScript(
						...
						function() {
							jQuery('.tw_button')
									.before(
											jQuery('<g:plusone count="true" size="medium"></g:plusone>'));
						});
	}
}
		...
		window.fbAsyncInit = function() {
			try {
				...
				FB.Event.subscribe('edge.create', function(targetUrl) {
					_gaq
							.push([ '_trackSocial', 'facebook', 'like',
									targetUrl ]);
				});
				FB.Event.subscribe('edge.remove', function(targetUrl) {
					_gaq
							.push([ '_trackSocial', 'facebook', 'unlike',
									targetUrl ]);
				});
				FB.Event.subscribe('message.send', function(targetUrl) {
					_gaq
							.push([ '_trackSocial', 'facebook', 'send',
									targetUrl ]);
				});
			} catch (e) {
				// do nothing
...
function addPjirc() {
	...
	if (navigator.appName == "Microsoft Internet Explorer") {
		...
		$pjicTag
				.attr('codebase',
						'http://java.sun.com/update/1.6.0/jinstall-6-windows-i586.cab#Version=6,0,0,0');
	}
...
function toggleTinyUrl() {
	...
	getTinyURL(url, function(tinyurl) {
		var $twitturl = "http://twitter.com/home?status="
